...
export default function CampaignPortal() {
    ...
    const sendRewardMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries('getRefereeWithCampaignDetails');
        },
        onError: (error) => {
    ...
    const handleSendReward = (reward) => {
        ...
        if (reward?.reward?.rewardType === 'COUPON' || reward?.reward?.rewardType === 'GIFTCARD') {
            ...
            if (reward?.reward?.method === 'add-later' && !reward?.reward?.code) {
    ...
    return (
        <div>
                                    ...
                                    <TableRow key={reward.id} className="border-t border-gray-100">
                                        ...
                                        <TableCell className="py-3 text-right">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="w-[200px]">
                                                    <DropdownMenuItem onClick={() => handleSendReward(reward)}>
                                                        Send reward
                                                    </DropdownMenuItem>
            ...
            {/* Dialog for entering coupon/gift card code */}
            <Dialog open={codeDialogOpen} onOpenChange={setCodeDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Enter {currentReward?.reward?.rewardType === 'COUPON' ? 'Coupon' : 'Gift Card'} Code</DialogTitle>
                        <DialogDescription>
                            Please enter the {currentReward?.reward?.rewardType === 'COUPON' ? 'coupon' : 'gift card'} code to be sent to the recipient.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                            ...
                            <Input
                                ...
                                placeholder={`Enter ${currentReward?.reward?.rewardType === 'COUPON' ? 'coupon' : 'gift card'} code`}
                            />
                        </div>
                    </div>
                    <DialogFooter>
                        ...
                        <Button onClick={handleSubmitCode} disabled={sendRewardMutation.isLoading}>
                            {sendRewardMutation.isLoading ? "Sending..." : "Send Reward"}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
